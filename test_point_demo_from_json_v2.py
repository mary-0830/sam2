...
image_dir = "/data3/ljj/proj/vggt/examples/kitchen"  # 修改为文件夹路径
save_dir = f"test_output/{image_dir.rsplit('/', 1)[-1]}_1point2box/image_masks"
...
# 获取 id=1 的框坐标
id_1_item = next(item for item in data if item["id"] == "1")
top_left = np.array(id_1_item["points"][0])  # 左上角点
bottom_right = np.array(id_1_item["points"][1])  # 右下角点
box_coords = np.concatenate([top_left, bottom_right])  # [x1, y1, x2, y2]
...
for item in data:
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        masks, scores, logits = predictor.predict(
            box=box_coords[None, :],  # 使用从 id=1 映射的框
    ...
    show_masks(image, masks, scores, save_dir, input_labels=input_label, borders=False, base_name=base_name, custom_color=[30/255, 255/255, 144/255], box_coords=box_coords)
